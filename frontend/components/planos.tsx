        ...
        <div className="sm:w-full md:w-full lg:w-full max-w-6xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-10 p-20 lg:p-6">
            {response?.planos.map((plano, index) => (
              <Card 
                key={index} 
                className={`sm:w-full md:w-full lg:w-full bg-[#2f2f2f] rounded-2xl border-none ${selectedPlan === plano ? "bg-neutral-600" : ""}`}
              >
                ...
                <CardContent>
                    ...
                    <div className="flex justify-center">
                      <CheckoutPlano plano={plano} />
                    </div>      
                    <CardDescription 
                      className="text-white font-semibold text-justify text-wrap md:h-36 lg:h-96" 
